...
public class ProjetoController {
    ...
    @GetMapping
    public ResponseEntity<Page<Projeto>> listarProjetos(@PageableDefault(size=5) Pageable paginacao){
        return ResponseEntity.status(HttpStatus.OK).body(projetoRepository.findAll(paginacao));
    }

    @GetMapping("/{id}")
    public ResponseEntity<Projeto> obterProjeto(@PathVariable("id") Long idProjeto){
    ...
    public ResponseEntity<Optional<List<Projeto>>> obterProjetosPorResponsavelTecnico(@PathVariable("idUsuario") Long idUsuario){
